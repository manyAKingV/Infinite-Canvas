...
function resultMediaUrls(result){
    ...
    const add = value => {
        ...
        if(typeof value === 'object'){
            ...
            ['image_items','media_items','items','outputs','videos','images','urls','data','result','content','files'].forEach(key => add(value[key]));
    ...
    add(result);
    ['image_items','media_items','items','outputs','videos','audios','texts','files','images','urls','data','result','output','url','content','result_url'].forEach(key => add(result?.[key]));
    const seen = new Set();
