...
async def save_remote_video_to_output(url, prefix="video_", category="output", provider=None, _seen=None):
    if not url:
        return ""
    if url.startswith("/output/") or url.startswith("/assets/"):
        return url
    video_exts = set(VIDEO_MEDIA_EXTS)
    ...
    if url in seen:
        return ""
    seen.add(url)
    clean_path = urllib.parse.unquote(parsed.path or "")
    ...
    try:
        ...
        headers = video_download_headers(provider) if (provider and (url_matches_provider_host(url, provider) or is_video_result_endpoint(url))) else {
        ...
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, headers=headers) as client:
            ...
            if "application/json" in content_type or (not looks_like_video_bytes(data) and data.lstrip()[:1] in (b"{", b"[")):
                try:
                    payload = response.json()
                except Exception as exc:
                    raise RuntimeError(f"unexpected video json: {exc}") from exc
                nested_urls = video_output_urls(payload) if isinstance(payload, dict) else []
                media_nested = [item for item in nested_urls if looks_like_video_media_url(item) and item not in seen]
                other_nested = [item for item in nested_urls if item not in seen and item not in media_nested]
                for nested in media_nested + other_nested:
                    saved = await save_remote_video_to_output(nested, prefix=prefix, category=category, provider=provider, _seen=seen)
                    if saved:
                        return saved
                raise RuntimeError(f"unexpected video json: {str(payload)[:400]}")
            ext = clean_ext
        ...
        try:
            ...
            pass
        if is_video_result_endpoint(url):
            raise HTTPException(status_code=502, detail=f"视频已生成，但下载结果失败：{e}") from e
        return url
...
VIDEO_MEDIA_EXTS = (".mp4", ".webm", ".mov", ".m4v", ".avi", ".mkv", ".flv")
VIDEO_URL_KEYS = (
    "url", "video_url", "videoUrl", "mp4_url", "mp4Url",
    "output", "output_url", "outputUrl", "download_url", "downloadUrl",
    "result_url", "resultUrl", "file_url", "fileUrl", "media_url", "mediaUrl",
...
VIDEO_CONTAINER_KEYS = (
    ...
    "content", "files", "file", "output",
)

def looks_like_video_media_url(value):
    text = str(value or "").strip()
    if not text.startswith(("http://", "https://", "/output/", "/assets/")):
        return False
    path = urllib.parse.unquote(urllib.parse.urlparse(text).path or "").lower()
    return any(path.endswith(ext) for ext in VIDEO_MEDIA_EXTS)

def is_video_result_endpoint(value):
    text = str(value or "").strip()
    if not text.startswith(("http://", "https://")) or looks_like_video_media_url(text):
        return False
    path = urllib.parse.urlparse(text).path.rstrip("/").lower()
    name = path.rsplit("/", 1)[-1] if path else ""
    return name in {"result", "download", "content", "file"} or path.endswith("/result") or "/download" in path

def looks_like_video_bytes(data):
    head = data[:64] if isinstance(data, (bytes, bytearray)) else b""
    if len(head) < 12:
        return False
    return (
        head[4:8] == b"ftyp"
...
def url_matches_provider_host(url, provider):
    if not provider:
        return False
    ...
    if "://" not in base:
        base = f"https://{base}"
    return urllib.parse.urlparse(url).netloc.lower() == urllib.parse.urlparse(base).netloc.lower()

def video_download_headers(provider=None):
    if provider:
        headers = api_headers(json_body=False, provider=provider)
    else:
        headers = {}
    headers["User-Agent"] = "ComfyUI-API-Modelscope/1.0"
    headers["Accept"] = "video/*,application/octet-stream,application/json;q=0.9,*/*;q=0.8"
    return headers
...
def video_output_urls(raw):
    ...
    for node in list(candidates):
        if not isinstance(node, dict):
            continue
        for key in ("result", "results", "files", "content", "output"):
            value = node.get(key)
    ...
    for url in urls:
        if isinstance(url, str) and url and url not in deduped:
            deduped.append(url)
    media = [url for url in deduped if looks_like_video_media_url(url) or url.startswith("/output/") or url.startswith("/assets/")]
    results = [url for url in deduped if is_video_result_endpoint(url)]
    others = [url for url in deduped if url not in media and url not in results]
    return media + results + others

def video_task_status(raw):
    if not isinstance(raw, dict):
        return ""
    nodes = [raw]
    for key in ("data", "detail", "output", "result", "content"):
        value = raw.get(key)
        if isinstance(value, dict):
            nodes.append(value)
    for node in nodes:
        status = node.get("status") or node.get("task_status") or node.get("state")
        if status:
...
def video_submit_url_candidates(provider, base_url):
    ...
    # ✅ 修改：使用文档中的正确路径 /v1/video/generations
...
def video_task_url_candidates(provider, base_url, task_id, submit_url=""):
    ...
    # 默认分支：优先尝试 /v1/video/jobs/（aiapbot.com / MiniMax H3）
    quoted_id = urllib.parse.quote(str(task_id), safe="")
    return [
        f"{base_url}/v1/video/jobs/{quoted_id}",
        f"{base_url}/v1/jobs/{quoted_id}",
        f"{base_url}/v1/video/generations/{quoted_id}",
        f"{base_url}/v1/videos/{quoted_id}",
        f"{base_url}/v1/tasks/{quoted_id}",
        f"{base_url}/v1/video/jobs/{quoted_id}/result",
    ]
...
async def resolve_video_task_payload(client, provider, raw):
    """If the task JSON only has an API result endpoint, fetch it so we can surface a real video URL."""
    if not isinstance(raw, dict):
        return raw
    if any(looks_like_video_media_url(url) or str(url).startswith(("/output/", "/assets/")) for url in video_output_urls(raw)):
        return raw
    result_urls = [url for url in video_output_urls(raw) if is_video_result_endpoint(url)]
    if not result_urls:
        extra = raw.get("result_url") or raw.get("resultUrl")
        data = raw.get("data") if isinstance(raw.get("data"), dict) else {}
        extra = extra or data.get("result_url") or data.get("resultUrl")
        if extra:
            result_urls = [str(extra).strip()]
    for url in result_urls:
        if not url:
            continue
        try:
            response = await client.get(url, headers=video_download_headers(provider))
            if response.status_code >= 400:
                continue
            content_type = (response.headers.get("Content-Type") or "").lower()
            body = response.content or b""
            if "json" in content_type or (not looks_like_video_bytes(body) and body.lstrip()[:1] in (b"{", b"[")):
                extra_json = response.json()
                if isinstance(extra_json, dict):
                    merged = {**raw, **extra_json}
                    if any(looks_like_video_media_url(item) for item in video_output_urls(merged)):
                        return merged
                    if video_output_urls(extra_json):
                        return merged
            elif looks_like_video_bytes(body) or "video/" in content_type or "octet-stream" in content_type:
                # Bytes are downloaded later via result_url; keep the original payload.
                return raw
        except Exception:
            continue
    return raw
...
async def wait_for_video_task(client, provider, task_id, submit_url=""):
    ...
    while time.monotonic() < deadline:
        ...
        task_data = (
            ...
            else raw.get("output") if isinstance(raw.get("output"), dict)
            else raw
        )
        status = video_task_status(raw)
        if status in VIDEO_TASK_SUCCESS_STATUSES:
            return await resolve_video_task_payload(client, provider, raw)
        # 部分上游（如玉玉API）status 字段非标准或为空，但已经返回了视频 URL ——
        # 只要不是明确的失败状态，且拿到了真实视频地址，就直接当成功处理。
        urls = video_output_urls(raw)
        if status not in VIDEO_TASK_FAILURE_STATUSES and any(
            looks_like_video_media_url(url) or url.startswith("/output/") or url.startswith("/assets/")
            for url in urls
        ):
            return await resolve_video_task_payload(client, provider, raw)
